...
public class UserDetailServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		displayView(request, response,
				UsersManager.getInstance().getByID(Integer.parseInt(request.getParameter("id"))));
	}
	...
	private void displayView(HttpServletRequest request, HttpServletResponse response, User user) throws IOException {
		...
		if (user != null) {
			out.println("<h1>User " + user.getName() + " " + user.getSurname()
					+ "</h1><form action='update' method='POST'><table>");
		...
		out.println("</br><a href='list'>Retour à la liste</a></body></html>");
	}
